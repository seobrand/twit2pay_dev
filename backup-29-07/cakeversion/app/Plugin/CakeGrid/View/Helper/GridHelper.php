...
//        );
	...
	/*	if($this->__columns['Start_Date']['title']=='Start  Date' || $this->__columns['End_Date']['title']=='End Date')
			{
				$rowColumns[] = date('d-M-Y',strtotime($this->__generateColumn($result, $column)));
		...
		*/
		...
		if(strpos($ColumnValuePath,'start_date') || strpos($ColumnValuePath,'end_date') || strpos($ColumnValuePath,'date_of_birth') || strpos($ColumnValuePath,'date'))
		{
		...
		}elseif(strpos($ColumnValuePath,'amount'))
		{
		
			$value = Set::extract($column['valuePath'], $result);
			$value = array_pop($value).'&nbsp;&nbsp; <img src="'.$this->webroot.'/img/ruppies.jpg" width="12" height="12"/>';
