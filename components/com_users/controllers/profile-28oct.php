	...
	public function update1(){
		...
		if ( $crm_customer_id != 0 )
		{
		 ...
		 if($rescnt > 1){
			...
			$db->query();
			//save logs
			$q1 = "INSERT INTO  #__shipping_logs (`user_id`, `cust_id`, `name`, `s_addr`, `s_city`, `s_state`, `s_zip`, `request_string`, `response`, `result_key`, `result_data`, `modified_date`, `type`) VALUES ('$userId', '$crm_customer_id', '$uName', '".$data['profile']['address1']."', '".$data['profile']['city']."', '".$data['profile']['region']."', '".$data['profile']['postal_code']."', '$request', '$result', '$result_key', '$result_data', '".time()."', 'update')";
			$db->setQuery($q1);
			$db->query();
					 	...
					 	if($ses_var[0]->username == $tuserName){
					 		...
					 		if($trcnt == 0){
								...
								<transactions>
					 		    ...
					 		    $q = "INSERT INTO #__transactions (`tweet_id`, `twit_log_id`, `transaction_id`, `replyed_user_id`, `replyed_usr_s_name`, `status`, `request_string`, `response`, `response_text`, `transaction_date`,`hastag`, `price`) VALUES ('$twi->id_str', '$twit_logId', '$transaction_id', '$userId', '$userName', '$transaction', '$request', '$result', '$response_text', now(), '$name', '$pro_price')";
	...
	public function update2(){
		...
		* */
		...
		if($crm_customer_id !=0 && $pos === false){
		 ...
		 $db->query();
		 //save logs
		 $q1 = "INSERT INTO  #__cc_logs (`user_id`, `cust_id`, `cc_type`, `cc_name`, `cc_number`, `cc_exp_month`, `cc_exp_year`, `cc_cvv`, `request_string`, `response`, `result_key`, `result_data`, `modified_date`, `type`) VALUES ('$user_id', '$crm_customer_id', '".$cc['cardtype']."', '".$cc['name_on_card']."', '".$cc_no."', '".$cc['expmonth']."', '".$cc['expyear']."', '".$cc['cvv']."', '$send', '$result', '$result_key', '$result_data', '".time()."', 'update')";
		 $db->setQuery($q1);
		 $db->query();
